    ...
    try {
      ...
      if (!response.ok) {
        if (attempt === maxRetries) {
          console.warn(
            `  ⚠️ Could not fetch schema for ${styleId}: ${response.status}`,
          );
          return null;
        }
...
function parseSchemaToCategories(schema: any): DiceBearCategory[] {
  ...
  for (const [key, prop] of Object.entries(properties)) {
    ...
    const isColorField =
      ...
      (prop.type === 'array' &&
        prop.default?.every?.((v: string) => /^[a-fA-F0-9]{6}$/.test(v)));
    ...
    if (isColorField) {
      ...
      if (Array.isArray(prop.examples)) {
        prop.examples.flat().forEach((c: string) => {
          if (c && c !== 'transparent' && /^[a-fA-F0-9]{3,8}$/.test(c))
            colorSet.add(c);
        });
      }
...
async function main() {
  ...
  const styleModel = app.get<Model<DiceBearStyleDocument>>(
    getModelToken(DiceBearStyle.name),
  );
  ...
  for (const style of DICEBEAR_STYLES) {
    ...
    console.log(
      `  ✅ ${style.name}: ${categories.length} categories, ${categories.reduce((sum, c) => sum + c.options.length, 0)} options`,
    );
    synced++;
  }
