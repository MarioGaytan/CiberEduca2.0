...
export class DashboardService {
  constructor(
    ...
    @InjectModel(TestAttempt.name) private readonly attemptModel: Model<TestAttemptDocument>,
  ) {}
  ...
  async getStatsForUser(user: AuthUser): Promise<DashboardStats> {
    ...
    const [totalWorkshops, approvedWorkshops, inReviewWorkshops, draftWorkshops] = await Promise.all([
      this.workshopModel.countDocuments({ schoolId }).exec(),
      this.workshopModel.countDocuments({ schoolId, status: 'approved' }).exec(),
      this.workshopModel.countDocuments({ schoolId, status: 'in_review' }).exec(),
  ...
  private async getAdminStats(schoolId: string) {
    const [totalUsers, students, teachers, reviewers, admins, experienceManagers] = await Promise.all([
      ...
      this.userModel.countDocuments({ schoolId, role: Role.ExperienceManager }).exec(),
    ]);

    const pendingRequests = await this.workshopModel.countDocuments({
      schoolId,
      $or: [
  ...
  private async getTeacherStats(userId: string, schoolId: string) {
    ...
    const testsCreated = await this.testModel.countDocuments({
      createdByUserId: userId,
    }).exec();

    // Get pending grades (attempts needing manual review)
    const pendingGrades = await this.attemptModel.countDocuments({
      gradedByUserId: null,
  ...
  private async getReviewerStats(schoolId: string) {
    const pendingReview = await this.workshopModel.countDocuments({
      schoolId,
      status: 'in_review',
    }).exec();
    ...
    const reviewedThisWeek = await this.workshopModel.countDocuments({
      schoolId,
      status: 'approved',
